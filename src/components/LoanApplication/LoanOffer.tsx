...
const LoanOffer: React.FC<LoanOfferProps> = ({ onNext }) => {
  ...
  const handleNext = () => {
    if (selectedOption) {
      if (selectedPurpose === 'Business') {
        if (!businessRegistrationCertificate || !idFront || !idBack) {
          return;
        }
      }

      onNext(selectedAmount, selectedPurpose, selectedPeriod, selectedOption.savingsDeposit, {
        businessRegistrationCertificate: businessRegistrationCertificate || undefined,
      ...
      <div className="bg-white rounded-lg p-6 shadow-sm">
            ...
            <div className="space-y-4">
              <div className="space-y-1">
                ...
                <input
                  type="file"
                  required
                  onChange={(e) => setBusinessRegistrationCertificate(e.target.files?.[0] || null)}
              ...
              <div className="space-y-1">
                <label className="block text-sm font-medium text-gray-700">ID Front Image</label>
                <input
                  type="file"
                  accept="image/*"
                  required
              ...
              <div className="space-y-1">
                <label className="block text-sm font-medium text-gray-700">ID Back Image</label>
                <input
                  type="file"
                  accept="image/*"
                  required
        ...
        <div className="mt-8 flex justify-between">
          ...
          <button
            ...
            disabled={
              selectedPurpose === 'Business' &&
              (!businessRegistrationCertificate || !idFront || !idBack)
            }
